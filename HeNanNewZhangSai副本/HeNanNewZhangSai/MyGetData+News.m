...
#import "MyGetData+News.h"

#import "Televise.h"
@implementation MyGetData (News)
...
+(void)getNewsAppsWithper:(NSInteger)per page:(NSInteger)pageNum categoty:(NSInteger)categoty handler:(GetDataHandler)handle{
    ...
    [[MyUrlsession defaultSession]accessServerWithUrlStr:URLStr andHandler:^(NSData *data, NSError *error) {
        ...
        if (!error) {
            ...
            //解析数据到model中。
            Televise *televisemodel =[[Televise alloc]initWithData:data error:&error];
            if (!error) {
                NSLog(@"解析成功");
            }else{
                NSLog(@"解析失败");
            }
            handle(televisemodel.data,nil,categoty);
        }else{
            NSLog(@"error%@",error);
            handle(nil,error,0);
        }
